...
class MultiHeadAttention(nn.Module):
    ...
    多头注意力机制模块
    """
    def __init__(self,d_model, num_heads):#d_model 词的维度
        super(MultiHeadAttention, self).__init__()
        assert d_model % num_heads == 0, "d_model 必须能被头数整除"
        ...
        self.W_o = nn.Linear(d_model, d_model) #W_o：输出投影层，将多头拼接后的结果映射回 d_model 维度。
    ...
    def scaled_dot_product_attention(self, Q, K, V, mask=None):
        ...
        # 2. 应用掩码 (如果提供)
        # 掩码：掩码就是“告诉模型：有些位置的信息你不能看，假装它们不存在
        if mask is not None:
